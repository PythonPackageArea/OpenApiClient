...
class Settings:
    keys: Keys = Keys
    settings_read: SettingsRead = SettingsRead
    settings_update: SettingsUpdate = SettingsUpdate
    ...
    async def get_value(
        self,
        key_path: Literal['proxies', 'device_tokens']
    ) -> Optional[str]:
    ...
    async def update_setting(
        self,
        key_query: Literal['proxies', 'device_tokens'],
        value_body: str = models.NOTSET
    ) -> SettingsRead:
    
        path = f'/settings/{id_path}'.rstrip()
        params = self._prepare_params(locals())
        data = self._prepare_body_data(locals())
        files = self._prepare_files(locals())
